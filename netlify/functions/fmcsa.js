...
exports.handler = async function(event, context) {
  ...
  try {
    // Primary: lookup by docket number (MC#)
    const raw = await fmcsaRequest(`/carriers/docket-number/${mc}?webKey=${FMCSA_KEY}`);
 
    // Log raw response for debugging
    console.log('FMCSA raw response:', JSON.stringify(raw).slice(0, 500));
 
    // Handle different response shapes
    let carrier = null;
    if (raw?.content?.carrier) {
      carrier = raw.content.carrier;
    } else if (raw?.content?.carrierBasics) {
      carrier = raw.content.carrierBasics;
    } else if (Array.isArray(raw?.content) && raw.content.length > 0) {
      carrier = raw.content[0].carrier || raw.content[0];
    } else if (raw?.carrier) {
      carrier = raw.carrier;
    }
    ...
    if (!carrier) {
      ...
      console.log('FMCSA alt response:', JSON.stringify(raw2).slice(0, 500));
    ...
    if (!carrier) {
      return {
        ...
        body: JSON.stringify({
          error: `No carrier found for MC# ${mc}. The MC number may be inactive or not registered with FMCSA.`,
          debug: JSON.stringify(raw).slice(0, 200)
        })
      };
    }
 
    const authorityActive = carrier.allowedToOperate === 'Y' || carrier.operatingStatus === 'AUTHORIZED FOR HHG' || carrier.operatingStatus === 'AUTHORIZED FOR PROPERTY';
    const insuranceOnFile = carrier.bipdInsuranceOnFile === 'Y' || carrier.cargoInsuranceOnFile === 'Y' || carrier.bipdInsuranceRequired === 'N';
    const safetyRating = carrier.safetyRating || carrier.safetyReviewType || '';
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        catch(e) { reject(new Error('FMCSA response parse error: ' + data.slice(0, 200))); }
      });
    });
    req.on('error', reject);
